...
function Channel() {
            ...
            if (res?.data) {
                ...
                setError(
                    <GuestComponent
                        ...
                        subtitle="There is no channel for the given username. Check the username again."
                        icon={<span className="w-full h-full flex items-center p-4"><FiVideoOff className="w-28 h-28" /></span>}
                        guest={false}
                    />
                );
            }
    ...
    const toggleSubscribe = async () => {
        try {
            const response = await axiosInstance.post(`/subscriptions/c/${profile._id}`);
    ...
    return profile ? (
        <section className="relative w-full pb-[70px] sm:ml-[70px] sm:pb-0 lg:ml-0">
            <div className="relative min-h-[150px] w-full pt-[20%]">
                <div className="absolute inset-0 overflow-hidden bg-gradient-to-b from-transparent to-black opacity-40">
                    <img src={profile?.coverImage || viewcell_cover} alt="user" className="object-cover w-full h-full" />
                </div>
            </div>

            <div className="px-4 pb-4">
                <div className="flex flex-wrap gap-6 pb-4 pt-6">
                    <span className="relative -mt-16 inline-block h-32 w-32 shrink-0 overflow-hidden rounded-full border-4 border-white shadow-xl">
                        <img src={profile?.avatar} alt="user avatar" className="h-full w-full object-cover" />
                    </span>
                    <div className="mr-auto inline-block">
                        <h1 className="font-bold text-2xl text-white">{profile?.fullName}</h1>
                        ...
                        {status === true ? (
                            userData?.username === profile?.username ? (
                                <Button
                                    ...
                                    className="mr-1 flex items-center font-semibold transition-all duration-150 ease-in-out active:translate-x-[5px] active:translate-y-[5px] rounded-md hover:bg-green-600"
                            ...
                            <>
                                ...
                                <Button
                                    ...
                                    className="flex items-center rounded-lg hover:bg-green-700"
                ...
                <ul className="no-scrollbar sticky top-0 bg-zinc-950 z-[2] flex gap-x-4 overflow-auto border-b-2 border-gray-400 py-2">
                    <li className="w-full">
                        <NavLink
                            to=""
                            end
